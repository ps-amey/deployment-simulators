...
COMMAND_SIGNALS = {
    0: ("SA1", "main", "SA1_HDRM_MAIN"),
    1: ("SA2", "main", "SA2_HDRM_MAIN"),
    2: ("SA1", "redundant", "SA1_HDRM_REDN"),
    3: ("SA2", "redundant", "SA2_HDRM_REDN"),
    4: ("RA", "main_1", "RA_HDRM_MAIN_1"),
    5: ("RA", "main_2", "RA_HDRM_MAIN_2"),
    6: ("RA", "redundant_1", "RA_HDRM_RED_1"),
    7: ("RA", "redundant_2", "RA_HDRM_RED_2"),
}

# For command GPn, voltage feedback uses obc_ext_adc GPn and current feedback
# uses obc_ext_adc GP(n + 8), as defined by HIL_Connections.xlsx.
EXT_ADC_SIGNALS = {
    pin: {
        "voltage_pin": pin,
        "current_pin": pin + 8,
    }
    for pin in COMMAND_SIGNALS
}

# Corrected OBC DI harness mapping supplied for this bench.
FEEDBACK_SIGNALS = {
    "SA1": (26, "OB_DTM_17", "SA1_HDRM_STATUS"),
    "SA2": (20, "OB_DTM_18", "SA2_HDRM_STATUS"),
    "RA": (21, "OB_DTM_19", "RA_HDRM_STATUS"),
}

# Nominal deployment pulse widths. Main and redundant are physical paths;
# normal versus extended selects the expected duration on either path.
SA_NORMAL = 50.0
SA_EXTENDED = 80.0
RA_NORMAL = 100.0
RA_EXTENDED = 200.0
...
SA_MILESTONE_TIMEOUT_S = 300.0
RA_MILESTONE_TIMEOUT_S = 500.0
...
def build_ext_adc_init_code() -> str:
    return "\n".join(
        [
            "from machine import Pin, mem32",
            "for pin_id in range(16):",
...
def build_ext_adc_update_code(previous_mask: int, new_mask: int) -> str:
    changed = (int(previous_mask) ^ int(new_mask)) & 0xFFFF
...
def build_feedback_code(states: dict[str, bool]) -> str:
    values = tuple(
        (FEEDBACK_SIGNALS[name][0], 1 if states.get(name, False) else 0)
        for name in ("SA1", "SA2", "RA")
    )
    return "\n".join(
        [
            ...
            "print('SARA_FEEDBACK=' + repr(values))",
        ]
    )
    ...
    for attempt in range(1, max(1, int(attempts)) + 1):
        try:
            ...
            if type(observed_mask) is not int:
                raise RuntimeError(
                    f"invalid LOW verification value {observed_mask!r}"
                )
            if observed_mask == 0:
                return
...
def parse_edge_payload(payload: str) -> EdgeSnapshot:
    ...
    for entry in falling_raw:
        if (
            ...
            or entry[0] not in COMMAND_SIGNALS
            or entry[1] < 0
        ):
...
def ext_adc_mask_for_state(
    ...
    *,
    ...
    active = int(command_state) & 0xFF
    return (active if voltage_enabled else 0) | (
        active << 8 if current_enabled else 0
    )
...
def expected_width_for_command(
    ...
    deployment_unit = deployment_unit_for_command(pin)
    mode = (
        deployment_modes[deployment_unit]
        if deployment_modes is not None
        else "main"
    )
    extended = mode in EXTENDED_PULSE_MODES
    if deployment_unit.startswith("SA"):
        return SA_EXTENDED if extended else SA_NORMAL
    return RA_EXTENDED if extended else RA_NORMAL
...
def apply_pulses(
    ...
    *,
    ...
    deployment_gate: (
        Callable[[str, str, int | None], tuple[bool, dict[str, bool]]] | None
    ) = None,
) -> tuple[dict[str, bool], list[dict[str, object]]]:
    new_states = dict(states)
    events: list[dict[str, object]] = []
    for pulse in pulses:
        deployable, path, signal_name = COMMAND_SIGNALS[pulse.pin]
        deployment_unit = deployment_unit_for_command(pulse.pin)
        configured_mode = (
            deployment_modes[deployment_unit]
            if deployment_modes is not None
            else "main"
        )
        expected_ms = expected_width_for_command(pulse.pin, deployment_modes)
        error_ms = pulse.width_ms - expected_ms
        width_valid = abs(error_ms) <= width_tolerance_ms
        accepted = width_valid or not strict_width
        deployment_milestones: dict[str, bool] = {}
        if accepted and deployment_gate is not None:
            deployment_condition_met, deployment_milestones = deployment_gate(
                deployment_unit,
                path,
                pulse_attempt_ids.get(pulse.pin) if pulse_attempt_ids else None,
            )
        else:
            deployment_condition_met = accepted
        unit_enabled = (
            deployment_enabled_by_unit[deployment_unit]
            if deployment_enabled_by_unit is not None
            else deployment_status_enabled
        )
        physical_status_enabled = (
            deployment_enabled_by_unit["RA1"]
            and deployment_enabled_by_unit["RA2"]
            if deployment_enabled_by_unit is not None and deployable == "RA"
            else unit_enabled
        )
        feedback_accepted = (
            accepted and physical_status_enabled and deployment_condition_met
        )
        if feedback_accepted:
            new_states[deployable] = True
        events.append(
            {
                ...
                "deployment_status_enabled": physical_status_enabled,
                "deployment_enable_states": (
                    dict(deployment_enabled_by_unit)
                    if deployment_enabled_by_unit is not None
                    else {deployment_unit: unit_enabled}
                ),
                "deployment_condition_met": deployment_condition_met,
                "deployment_milestones": deployment_milestones,
                "feedback_accepted": feedback_accepted,
                "feedback_pin": FEEDBACK_SIGNALS[deployable][0],
            }
        )
    return new_states, events


def configured_deployment_modes(args: argparse.Namespace) -> dict[str, str]:
    return {
        "SA1": args.sa1_mode,
        "SA2": args.sa2_mode,
        "RA1": args.ra1_mode,
...
def configured_deployment_enables(args: argparse.Namespace) -> dict[str, bool]:
    return {
        "SA1": args.sa1_deployment == "yes",
        "SA2": args.sa2_deployment == "yes",
        "RA1": args.ra1_deployment == "yes",
        "RA2": args.ra2_deployment == "yes",
    }


def deployment_unit_for_command(pin: int) -> str:
    deployable, path, _ = COMMAND_SIGNALS[pin]
    if deployable != "RA":
        return deployable
    return "RA1" if path.endswith("_1") else "RA2"


def selected_command_pins(deployment_modes: dict[str, str]) -> tuple[int, ...]:
    pins: list[int] = []
    for deployment, mode in deployment_modes.items():
        for path in PULSE_MODE_PATHS[mode]:
            pins.append(DEPLOYMENT_PATH_PINS[deployment][path])
...
def configured_main_red_pairs(
    ...
    """Return the main/redundant input pair for each combined deployment mode."""
    return {
        deployment: (
            DEPLOYMENT_PATH_PINS[deployment]["main"],
            DEPLOYMENT_PATH_PINS[deployment]["red"],
        )
        for deployment, mode in deployment_modes.items()
        if mode == "main-red"
    }
...
def qualified_command_state(
    ...
    """Suppress a main-red pair unless both of its inputs are currently HIGH."""
    qualified = 0
    for deployment, mode in deployment_modes.items():
        selected_paths = PULSE_MODE_PATHS[mode]
        selected_bits = tuple(
            1 << DEPLOYMENT_PATH_PINS[deployment][path]
...
def build_deployment_gate(
    ...
    *,
    ...
    """Track deployment milestones within the configured completion windows."""
    accepted_paths = {name: set() for name in FEEDBACK_SIGNALS}
    ...
    def required_signal_paths(deployment: str) -> set[str]:
        mode = deployment_modes[deployment]
        paths = PULSE_MODE_PATHS[mode]
        return {
            COMMAND_SIGNALS[DEPLOYMENT_PATH_PINS[deployment][path]][1]
            for path in paths
        }

    sa_requirements = {
        name: required_signal_paths(name) for name in ("SA1", "SA2")
    }
    ra_requirements = {
        name: required_signal_paths(name) for name in ("RA1", "RA2")
    }
    requirements = {**sa_requirements, **ra_requirements}

    def milestones_met(
        deployment_unit: str, path: str, attempt_id: int | None
    ) -> tuple[bool, dict[str, bool]]:
        deployable = "RA" if deployment_unit.startswith("RA") else deployment_unit
        now = monotonic_clock()
        timeout_s = (
            RA_MILESTONE_TIMEOUT_S
            if deployable == "RA"
            else SA_MILESTONE_TIMEOUT_S
        )
        started_at = milestone_started_at[deployable]
        if started_at is not None and now - started_at >= timeout_s:
            accepted_paths[deployable].clear()
            milestone_started_at[deployable] = None
        if (
            attempt_id is not None
            and latest_attempt_by_unit.get(deployment_unit) != attempt_id
        ):
            # A main-red pair must be completed by the same HIGH-overlap attempt.
            accepted_paths[deployable].difference_update(requirements[deployment_unit])
            latest_attempt_by_unit[deployment_unit] = attempt_id
        if not accepted_paths[deployable]:
            milestone_started_at[deployable] = now
        accepted_paths[deployable].add(path)
        if deployable == "RA":
            milestones = {
                name: required <= accepted_paths["RA"]
                for name, required in ra_requirements.items()
            }
            return all(milestones.values()), milestones
        complete = sa_requirements[deployable] <= accepted_paths[deployable]
        return complete, {deployable: complete}
...
def print_mapping() -> None:
    ...
    for pin, (_, path, name) in COMMAND_SIGNALS.items():
        print(f"  GP{pin:<2} {name:<20} {path}")
    print("Feedback Pico (to OBC inputs):")
    for name, (pin, obc_label, signal_name) in FEEDBACK_SIGNALS.items():
        print(f"  GP{pin:<2} {obc_label:<10} {signal_name} ({name})")
    print("External ADC Pico (V/I pulse feedback):")
    for pin, (_, path, name) in COMMAND_SIGNALS.items():
        mapping = EXT_ADC_SIGNALS[pin]
        print(
            f"  {name:<20} {path:<11} V=GP{mapping['voltage_pin']:<2} "
            f"I=GP{mapping['current_pin']}"
        )


def parse_dry_run_pulses(value: str) -> list[Pulse]:
    pulses: list[Pulse] = []
    for item in value.split(","):
        ...
        if pin not in COMMAND_SIGNALS or width_us < 0:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--log", type=Path, default=Path("sara_deployment_events.jsonl"))
    ...
    for deployment in ("SA1", "SA2", "RA1", "RA2"):
    ...
    parser.add_argument(
        ...
        required=True,
        choices=("yes", "no"),
        help="reproduce selected command pulses on obc_ext_adc GP0-GP7",
    )
    parser.add_argument(
        "--i-ch-feedback",
        required=True,
        choices=("yes", "no"),
        help="reproduce selected command pulses on obc_ext_adc GP8-GP15",
    )
    parser.add_argument("--dry-run-pulses", type=parse_dry_run_pulses, metavar="GP0:50,GP1:50", help="process synthetic pulses without opening Picos")
    return parser
...
def report_events(events: Sequence[dict[str, object]]) -> None:
    for event in events:
        ...
        if configured_mode == "ex-main":
            ...
            line_name = "REDUNDANT" if path.startswith("redundant") else "MAIN"
...
def run_dry(args: argparse.Namespace, event_log: EventLog) -> int:
    states = {name: False for name in FEEDBACK_SIGNALS}
    deployment_modes = configured_deployment_modes(args)
    deployment_enables = configured_deployment_enables(args)
    deployment_gate = build_deployment_gate(deployment_modes)
    selected_pins = set(selected_command_pins(deployment_modes))
    selected_pulses = [
    ...
    for pulse in ignored_pulses:
        print(
            f"Ignored GP{pulse.pin} ({COMMAND_SIGNALS[pulse.pin][2]}): not in "
            f"configured deployment paths {deployment_modes}"
        )
        event_log.write(
            "pulse_ignored",
            command_pin=pulse.pin,
            signal=COMMAND_SIGNALS[pulse.pin][2],
...
def run_stow_feedback(args: argparse.Namespace, event_log: EventLog) -> int:
    with RawRepl(args.feedback_pico) as feedback_pico:
        ...
        try:
            drive_low_and_verify(
                "status Pico",
                feedback_pico,
                tuple(FEEDBACK_SIGNALS[name][0] for name in ("SA1", "SA2", "RA")),
            )
        finally:
    ...
    print(
        "SA1, SA2 and RA feedback are STOWED/LOW and Pico latch verified. "
...
def run_hardware(args: argparse.Namespace, event_log: EventLog) -> int:
    stop = False
    ...
    deployment_modes = configured_deployment_modes(args)
    deployment_enables = configured_deployment_enables(args)
    capture_pins = selected_command_pins(deployment_modes)
    ...
    main_red_pairs = configured_main_red_pairs(deployment_modes)
    ...
    with ExitStack() as stack:
        ...
        if ext_adc_pico is not None:
            ext_adc_pico.execute_raw(build_ext_adc_init_code())
            drive_low_and_verify(
                "external-ADC Pico",
                ext_adc_pico,
                tuple(range(16)),
        ...
        simulator_deadline = time.monotonic() + SIMULATOR_TIMEOUT_S
        event_log.write(
            "started",
            ...
            while not stop:
                ...
                while b"\n" in stream_buffer:
                    ...
                    if EDGE_MARKER not in line:
                        continue
                    snapshot = parse_edge_payload(line.split(EDGE_MARKER, 1)[1])
                    ...
                    qualified_state = qualified_command_state(
                        snapshot.state_mask, deployment_modes
                    )
                    ...
                    if pulses:
                        ...
                        if feedback_pico is not None and feedback_changed:
                            ...
                            feedback_pico.execute_raw(build_feedback_code(states))
                    ...
                    for pulse in unpaired_main_red_pulses:
                        ...
                        print(
                            ...
                            f"{COMMAND_SIGNALS[pulse.pin][1].upper()} PULSE: "
                    ...
                    completed_single_path = any(
                        deployment_modes[deployment_unit_for_command(pulse.pin)]
                        != "main-red"
                        for pulse in pulses
                    )
                    if args.once and (
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.stow_feedback:
        return run_stow_feedback(args, event_log)
    if args.dry_run_pulses is not None:
        return run_dry(args, event_log)
    return run_hardware(args, event_log)
